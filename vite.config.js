    ...
    plugins: [
      ...
      tailwindcss(),
      {
        ...
        configureServer(server) {
          ...
          server.middlewares.use('/api/display/status', (req, res, next) => {
            if (req.method !== 'GET') return next();
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ mode: currentDisplayMode }));
          });

          // POST /api/display/extend
          server.middlewares.use('/api/display/extend', (req, res, next) => {
            if (req.method !== 'POST') return next();
          ...
          // POST /api/display/duplicate
          server.middlewares.use('/api/display/duplicate', (req, res, next) => {
            if (req.method !== 'POST') return next();
            exec('DisplaySwitch.exe /clone', (err) => {
